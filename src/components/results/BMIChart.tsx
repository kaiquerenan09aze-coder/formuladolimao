...
const BMIChart = ({ bmi }: BMIChartProps) => {
  ...
  return (
    <div className="flex flex-col items-center">
      <svg ref={svgRef} width="300" height="160"></svg>
      <div className="text-4xl font-bold text-primary mt-2">{bmi.toFixed(1)}</div>
      <div className={`text-sm font-semibold uppercase tracking-wider ${category.color}`}>
        {category.label}
      </div>
    </div>
  );
